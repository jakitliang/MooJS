 ...
 *
 * - 此版本作为jQuery插件运行
 * - This version of MooJS is runing as a jQuery plugin.
 *
 * @author Jakit
 * @copyright Jakit
 * @date 2016/4/15
...
(function ($) {
	...
	$.fn.Moo = function () {
		...
		return this.each(function () {
			...
			_t.mSelect = function (data, attr) {
				_Moo_listing.select(_t, data, attr);
			};

			_t.mTable = function (data, fields) {
				_Moo_listing.table(_t, data, fields);
			};

			_t.paging = function (total, pages, callback) {
				_Moo.paging(_t, total, pages, callback);
			};
		});
	};
})(window.jQuery);